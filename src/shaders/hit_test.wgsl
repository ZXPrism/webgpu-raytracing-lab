...
@group(0) @binding(4) var<storage, read_write> out_ray_array_length: atomic<u32>;
@group(0) @binding(5) var<storage, read_write> out_ray_array: array<Ray>;
@group(0) @binding(6) var<storage, read_write> out_indirect_args: IndirectArgs;
...
fn compute(
  ...
  if id == 0u {
    out_indirect_args = IndirectArgs(0u, 1u, 1u);
  }
}
